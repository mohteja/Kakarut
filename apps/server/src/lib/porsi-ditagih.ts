...
export const qtyDitagihSql = sql<number>`GREATEST(${saleItems.qty} - ${saleItems.qtyRefund}, 0)`;
 ...
 */
export const omzetDitagihSql = sql<number>`COALESCE(SUM(${saleItems.hargaSatuan} * GREATEST(${saleItems.qty} - ${saleItems.qtyRefund}, 0)), 0)`;

/** Jumlah porsi yang ditagih pada satu grup agregasi. */
export const sumQtyDitagihSql = sql<number>`COALESCE(SUM(GREATEST(${saleItems.qty} - ${saleItems.qtyRefund}, 0)), 0)`;

/** HPP yang ditagih: `hpp_satuan` tetap per porsi, dikalikan porsi yang jadi. */
export const hppDitagihSql = sql<number>`COALESCE(SUM(${saleItems.hppSatuan} * GREATEST(${saleItems.qty} - ${saleItems.qtyRefund}, 0)), 0)`;
