...
const TahapBody = z.object({
  ...
  /**
   * Identitas PERMINTAAN (bukan isinya) — tanpa ini `/tahap` tak punya cara
   * membedakan kiriman ulang dari permintaan kedua yang sah. Lihat alasan
   * lengkapnya di handler.
   */
...
function buatRuteTambahStok(tipe: JenisPengadaan) {
  return new Hono<AppEnv>()
    ...
    .post("/tahap/:fakturId", zValidator("json", TahapBody), async (c) => {
       ...
       *
       * CAS `(id, status, qty)` di bawah menjaga jalur "maju PENUH": statusnya
       * berubah, jadi kiriman kedua tak cocok lagi. Jalur SPLIT tidak: ia cuma
       * MENGURANGI qty baris induk tanpa menyentuh statusnya, dan permintaan
       * yang sama dikirim ulang membaca qty yang baru itu — lalu cocok, lalu
       * memotong lagi.
       *
       * Terukur pada faktur 8 pcs: "terima 3" yang terkirim dua kali
       * menghasilkan DUA baris 3 pcs masuk stok (saldo +6) dan sisa tugas
       * belanja menyusut dari 5 jadi 2. Barang yang tak pernah datang tercatat
       * datang, dan yang masih harus dibeli tercatat lebih sedikit daripada
       * yang sebenarnya.
       *
       * "Majukan 3 dari baris ini" memang tak bisa dibuat idempoten dari
       * isinya sendiri — sama seperti "tarik 100 ribu". Yang membedakan
       * kiriman ulang dari permintaan kedua yang sah hanyalah IDENTITAS
       * permintaannya. Maka `client_ref`, lewat helper yang sama dengan enam
       * jalur lain, bukan salinan baru.
       */
