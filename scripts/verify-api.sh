...
cek "stok masuk 10 sachet (Rp20.000) → saldo 10" "abs(V - 10) < 0.001" \
  "$(api "$OWNER" POST "/perlengkapan/$SB83/masuk" '{"qty":10,"total_harga":20000}' | jq .saldo)"
...
api "$OWNER" POST "/perlengkapan/$SP83/masuk" '{"qty":1}' > /dev/null
